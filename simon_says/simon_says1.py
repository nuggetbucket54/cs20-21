...
class Gamewin(Fl_Window):
    ...
    def rand_cb(self):
        if self.flag:
            ...
            Gamewin.clicked = []
            Fl.add_timeout(1.0, self.interval_to, [Gamewin.buttons[Gamewin.order[Gamewin.flashcount]],Gamewin.buttons[Gamewin.order[Gamewin.flashcount]].color()])
                #'''if but:
    ...
    def interval_to(self,butcolor):
        ...
        if Gamewin.flashcount < len(Gamewin.order):
            Fl.add_timeout(1.0,self.rand_cb)
